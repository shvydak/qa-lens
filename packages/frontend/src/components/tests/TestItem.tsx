...
export default function TestItem({
  ...
  const headline = test.title || test.description
  ...
  return (
    <div
      ...
      className={`group flex flex-col gap-0 rounded-xl border transition-all duration-500 ${STATUS_CARD[test.status]} ${highlighted ? 'ring-2 ring-indigo-400/60 ring-offset-2 ring-offset-gray-950' : ''} ${fading ? 'opacity-25' : 'opacity-100'}`}>
      <div className="flex items-start gap-3 px-4 pt-3 pb-2">
        <div className="mt-0.5 flex-shrink-0">{STATUS_ICON[test.status]}</div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-sm font-medium leading-snug ${STATUS_TITLE[test.status]}`}>
            {headline}
          </p>
      ...
      {/* Bottom action bar */}
      <div className="flex items-center justify-between gap-2 border-t border-gray-800/40 px-3 py-1.5">
        <div className="flex items-center gap-1">
          {(['pass', 'fail', 'skip'] as const).map((s) => {
            const isActive = test.status === s
            const styles = {
              pass: {
                active: 'bg-emerald-500/20 border-emerald-500/50 text-emerald-300',
                idle: 'border-transparent text-gray-600 hover:border-emerald-500/30 hover:text-emerald-400 hover:bg-emerald-500/10',
              },
              fail: {
                active: 'bg-red-500/20 border-red-500/50 text-red-300',
                idle: 'border-transparent text-gray-600 hover:border-red-500/30 hover:text-red-400 hover:bg-red-500/10',
              },
              skip: {
                active: 'bg-gray-700/60 border-gray-600/60 text-gray-300',
                idle: 'border-transparent text-gray-600 hover:border-gray-600/40 hover:text-gray-400 hover:bg-gray-700/30',
              },
            }
            return (
              <button
                key={s}
                type="button"
                onClick={() => onStatusChange(isActive ? 'not_tested' : s)}
                title={isActive ? `Unset ${s}` : s.charAt(0).toUpperCase() + s.slice(1)}
                className={`rounded-md border px-2 py-0.5 text-xs font-medium transition-all ${isActive ? styles[s].active : styles[s].idle}`}>
        ...
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
          {onEdit && (
            <button
              ...
              title="Edit"
              className="p-1 text-gray-700 hover:text-indigo-300 hover:bg-indigo-400/10 rounded transition-colors">
              <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
                <path
                  d="M7.2 1.6L9.4 3.8M2 9h2l5-5L7 2 2 7v2z"
                  stroke="currentColor"
                  strokeWidth="1.2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
