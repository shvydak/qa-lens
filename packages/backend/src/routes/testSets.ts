...
testSetActionsRouter.patch('/:testSetId', (req, res) => {
  ...
  if (status === 'passed') {
    try {
      ...
      return res.status(500).json({ error: err instanceof Error ? err.message : 'Failed to mark as passed' })
    }
  } else {
...
function toDto(row: unknown) {
  ...
  return {
    ...
    name: r.name,
    status: r.status,
    commitRanges: typeof r.commit_ranges === 'string' ? JSON.parse(r.commit_ranges) : r.commit_ranges,
    aiSummary: r.ai_summary ?? null,
    regressions: typeof r.regressions === 'string' ? JSON.parse(r.regressions as string) : (r.regressions ?? []),
    crossImpacts: typeof r.cross_impacts === 'string' ? JSON.parse(r.cross_impacts as string) : (r.cross_impacts ?? []),
