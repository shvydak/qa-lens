...
import {projectFromRow} from '../db/mappers.js'
...
projectsRouter.get('/', (_req, res) => {
  ...
  res.json({data: projects.map(projectFromRow)})
})
...
projectsRouter.post('/', (req, res) => {
  ...
  return res.status(201).json({data: projectFromRow(project)})
})
...
projectsRouter.get('/:id', (req, res) => {
  ...
  return res.json({data: projectFromRow(project)})
})
...
projectsRouter.patch('/:id', (req, res) => {
  ...
  return res.json({data: projectFromRow(updated)})
})
