...
**Test set deletion:** Plain `DELETE /api/test-sets/:id` removes history only; `?rewind=true` also recomputes each repo's cursor from the latest remaining closed test sets (`passed`, `failed`, `reviewed`, or `not_required`). UI always uses `?rewind=true` for closed test sets (no plain Delete option) — deleting a closed set without rewind creates a gap in analysis history.

**Close review checklist semantics:** Closing a test set as `reviewed` or `not_required` advances the cursor and converts tests that are not `pass`/`fail` to `skip`; preserve existing `pass` and `fail` statuses so progress/history stays accurate.
