...
- `AnalysisService.ts` — orchestrates the full analysis cycle: gather diffs from all repos in parallel → call AI → persist `TestSet` + `Test` rows in a single transaction. Tracks in-flight jobs in a `Map<projectId, job>` (in-memory, resets on restart). `closeTestSetReview()` updates `last_analyzed_commit_hash` for every repo in the test set's `commit_ranges` in a single transaction when a review is closed as `passed`, `failed`, or `not_required`.
...
**Key constraint:** When `PATCH /api/test-sets/:id` receives a closing status (`passed`, `failed`, or `not_required`), it must call `closeTestSetReview()` (not a plain UPDATE) to advance `last_analyzed_commit_hash` on all linked repos. This is the mechanism that defines "what's new" for the next analysis.

**Empty reviews:** When AI returns 0 tests, `AnalysisService.run()` still creates (or extends) a test set so the analyzed commit range is recorded. Initial 0-test sets get `is_empty_review=1`; the UI closes them through "Close Review" with `not_required` by default, which calls `closeTestSetReview()` and advances the cursor without running tests. Updates that add 0 tests still extend `commit_ranges` and insert an empty `analysis_run` (label "Update YYYY-MM-DD"). Frontend treats a set as empty-review only when `isEmptyReview && tests.length === 0`, so adding a manual test transitions it back to the normal flow without DB changes.
...
**Test set deletion:** Plain `DELETE /api/test-sets/:id` removes history only; `?rewind=true` also recomputes each repo's cursor from the latest remaining closed test sets (`passed`, `failed`, or `not_required`). UI always uses `?rewind=true` for closed test sets (no plain Delete option) — deleting a closed set without rewind creates a gap in analysis history.
